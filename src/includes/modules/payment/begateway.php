...
class begateway {
  ...
  function __construct() {
    ...
    $this->code = 'begateway';
    $this->title = MODULE_PAYMENT_BEGATEWAY_TEXT_TITLE;
    $this->public_title = MODULE_PAYMENT_BEGATEWAY_TEXT_PUBLIC_TITLE;
    $this->description = MODULE_PAYMENT_BEGATEWAY_TEXT_ADMIN_DESCRIPTION;
    $this->icon = DIR_WS_ICONS . 'begateway.png';
    $this->sort_order = MODULE_PAYMENT_BEGATEWAY_SORT_ORDER;
    $this->enabled = ((MODULE_PAYMENT_BEGATEWAY_STATUS == 'True') ? true : false);
  ...
  function update_status() {
    ...
    if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_BEGATEWAY_ZONE > 0) ) {
      $check_flag = false;
      $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_BEGATEWAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
  ...
  function selection() {
    ...
    if (isset($_SESSION['cart_begateway_id'])) {
      $order_id = substr($_SESSION['cart_begateway_id'], strpos($_SESSION['cart_begateway_id'], '-')+1);
      ...
      if (vam_db_num_rows($check_query) < 1) {
        ...
        unset($_SESSION['cart_begateway_id']);
      }
    }
  ...
  function confirmation() {
    ...
    if (isset($_SESSION['cartID'])) {
      ...
      if (isset($_SESSION['cart_begateway_id'])) {
        ...
        if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_begateway_id'], 0, strlen($cartID))) ) {
          $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
      ...
      if ($insert_order == true) {
        ...
        $_SESSION['cart_begateway_id'] = $cartID . '-' . $insert_id;
      }

      if ($insert_order == true) {
        $token = $this->_getPaymentToken($order);
        if (!$token['error']) {
          vam_redirect($token['url']);
        } else {
          die($token['message']);
        }
      }
    ...
    return array('title' => MODULE_PAYMENT_BEGATEWAY_TEXT_DESCRIPTION);
  }
  ...
  function check() {
    if (!isset($this->_check)) {
      $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_BEGATEWAY_STATUS'");
  ...
  function install() {
    ...
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_BEGATEWAY_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_BEGATEWAY_ALLOWED', '', '6', '2', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_BEGATEWAY_ID', '', '6', '3', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_BEGATEWAY_SECRET_KEY', '', '6', '4', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_BEGATEWAY_CHECKOUT_URL', '', '6', '5', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_BEGATEWAY_TRANSACTION_TYPE', 'False', '6', '6', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_BEGATEWAY_TEST_MODE', 'True', '6', '6', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_BEGATEWAY_SORT_ORDER', '0', '6', '7', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_BEGATEWAY_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
    vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_BEGATEWAY_ORDER_STATUS_ID', '0', '6', '9', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
  }
  ...
  function keys() {
    return array(
      'MODULE_PAYMENT_BEGATEWAY_STATUS', 'MODULE_PAYMENT_BEGATEWAY_ALLOWED',
      'MODULE_PAYMENT_BEGATEWAY_ID', 'MODULE_PAYMENT_BEGATEWAY_SORT_ORDER',
      'MODULE_PAYMENT_BEGATEWAY_ZONE', 'MODULE_PAYMENT_BEGATEWAY_SECRET_KEY',
      'MODULE_PAYMENT_BEGATEWAY_ORDER_STATUS_ID',
      'MODULE_PAYMENT_BEGATEWAY_TEST_MODE',
      'MODULE_PAYMENT_BEGATEWAY_CHECKOUT_URL',
      'MODULE_PAYMENT_BEGATEWAY_TRANSACTION_TYPE'
    );
  }

  function _getPaymentToken($order) {
    \BeGateway\Settings::$shopId = MODULE_PAYMENT_BEGATEWAY_ID;
    \BeGateway\Settings::$shopKey = MODULE_PAYMENT_BEGATEWAY_SECRET_KEY;
    \BeGateway\Settings::$checkoutBase = 'https://' . MODULE_PAYMENT_BEGATEWAY_CHECKOUT_URL;
    ...
    if (MODULE_PAYMENT_BEGATEWAY_TRANSACTION_TYPE == 'True') {
      $transaction->setAuthorizationTransactionType();
    }

    $transaction->setTestMode(MODULE_PAYMENT_BEGATEWAY_TEST_MODE == 'True');

    $notification_url = vam_href_link('begateway.php', '', 'SSL');
    ...
    $transaction->setSuccessUrl(vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL'));
    $transaction->setDeclineUrl(vam_href_link(FILENAME_CHECKOUT, '', 'SSL'));
    $transaction->setFailUrl(vam_href_link(FILENAME_CHECKOUT, '', 'SSL'));
    $transaction->setCancelUrl(vam_href_link(FILENAME_CHECKOUT, '', 'SSL'));
    $transaction->setLanguage($_SESSION['language_code']);
    $transaction->setTrackingId($order_id);
    $transaction->money->setAmount($order_sum);
    $transaction->money->setCurrency($curr['currency']);
    $transaction->setDescription(MODULE_PAYMENT_BEGATEWAY_ORDER_DESC . ' #' . $order_id);

    $firstname = (!isset($order->customer['firstname'])) ? null : $order->customer['firstname'];
    $lastname = (!isset($order->customer['lastname'])) ? null : $order->customer['lastname'];
    ...
    $state = (!isset($order->delivery["state"])) ? null : vam_get_zone_code($order->delivery['country']['id'], $order->delivery['zone_id'], $order->delivery['state']);
    $country = (!isset($order->delivery["country"])) ? null : $order->delivery["country"]["iso_code_2"];

    $phone = (!isset($order->customer['telephone'])) ? null : $order->customer['telephone'];
    $email = (!isset($order->customer['email_address'])) ? null : $order->customer['email_address'];

    $transaction->customer->setFirstName($firstname);
    $transaction->customer->setLastName($lastname);
    $transaction->customer->setAddress($street_address);
    $transaction->customer->setCountry($country);
    $transaction->customer->setCity($city);
    $transaction->customer->setZip($postcode);
